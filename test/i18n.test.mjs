...
test('в боте не осталось русского текста мимо словаря', () => {
  // Блочные комментарии режем тоже. Без этого русский внутри /* */ ломал парность кавычек:
  // поиск начинался от закрывающей кавычки предыдущей строки и хватал кусок кода целиком,
  // а сообщение об ошибке показывало не ту строку, которую надо править.
  const code = BOT.replace(/\/\*[\s\S]*?\*\//g, ' ').replace(/^\s*\/\/.*$/gm, '');
