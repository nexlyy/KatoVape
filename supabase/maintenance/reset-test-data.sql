...
-- ---------- остаток ----------
-- Товар остаётся, а количество обнуляем: цифры на полке тоже тестовые, и часть из них
-- завышена старым багом (выданный заказ не уменьшал склад). Дальше по договорённости остаток
-- заводит поставка: она же принесёт закупочную цену, и маржа будет точной с первого дня.
--
-- Если количество на полке уже верное и его надо сохранить, закомментируйте следующую строку.
